...
const PartnerElement = ({
  ...
  return (
    <li className="partner">
      <p className="partner__engines">
        {" "}
        {partner.parts_available.map((part, index) => (
          <React.Fragment key={index}>
            {part.name}
            {index < partner.parts_available.length - 1 && (
              <span className="partner__engines-dot"></span>
            )}
          </React.Fragment>
        ))}
      </p>
      <h2 className="partner__name" onClick={handleClick}>
        {partner.name}
      </h2>
      ...
      <div className="partner__block">
        <div className="partner__contacts">
          {partner.phone && (
            <a href="#" className="partner__phone" onClick={handlePhoneClick}>
              {partner.phone}
            </a>
          )}
          {partner.website && (
        ...
        partner.time_open_sunday ? (
          <div className="partner__open">
            {partner.time_open_weekdays ? (
              <p className="partner__open-time">
        ...
        <button className="route-button" onClick={handleRouteButton}>
          Маршрут
        </button>
      </div>
    </li>
